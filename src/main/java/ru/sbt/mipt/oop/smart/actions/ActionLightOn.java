...
public class ActionLightOn implements Actionable {

    @Override
    public void act(SmartDevice device) {
        if (device.getType() != SmartDeviceType.LIGHT) {
            System.out.printf("The device type '%s' does not match the event action 'Light on'%n", device.getType().toString());
            return;
        }
